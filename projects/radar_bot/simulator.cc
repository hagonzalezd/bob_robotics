...
int main()
{
    ...
    // The 0.25f 1.0f definitions are assigning floating numbers to the proportions. Then the max velocity for the robot should be avoided due to the high power of the SMC_G2 pololu
    ...
    try {
        while(true) {
	        ...
	        //robot.tank(1.0f, 1.0f); // This command sets forward movement, by assigning 1.0f max speed to both motors (left=1.0f*max_speed, right=1.0f*max_speed) 
	        //robot.tank(0.1f, 0.1f); // Velocity reduced since the SMC_G2 pololu for tudbot is accelerating too much at max
	        robot.tank(0.5f, 0.5f); // 0.1 was too slow

                const auto sleepEnd = std::chrono::system_clock::now() + std::chrono::seconds(1);
                while(std::chrono::system_clock::now() < sleepEnd) {
                    std::this_thread::sleep_until(sleepEnd);
		    std::cout << std::endl;
		    std::cout << "Moving FORWARD ACTION" << std::endl;
		    std::cout << std::endl;		    
                }
            }
            else {
                if(spikes[1] > spikes[0]) {
		    //robot.tank(0.25f, -0.25f); // This means left moves forward, right moves backward. So, turning right
  		    //robot.tank(0.1f, -0.1f); // This means left moves forward, right moves backward. So, turning right
  		    robot.tank(0.5f, -0.5f); // This means left moves forward, right moves backward. So, turning right

		    std::cout << std::endl;
		    std::cout << "Steering RIGHT ACTION" << std::endl;
		    std::cout << std::endl;
		    
                }
                else {
		    //robot.tank(-0.25f, 0.25f); // This means left moves backward, right moves forward. So, turning left
  		    robot.tank(-0.5f, 0.5f); // This means left moves backward, right moves forward. So, turning left

		    std::cout << std::endl;		    
		    std::cout << "Steering LEFT ACTION" << std::endl;
		    std::cout << std::endl;
